...
- 2026-05-25: Addressed `gh-review-hook` feedback by adding missing localized unregister strings, clearing the pending dialog project at confirmation claim time, and recording audit intent as `purge_requested` instead of claiming purge completion before deleting the project directory.
- 2026-05-25: Hook-fix validation passed: `git diff --check`; `rustfmt --check src/cli/project.rs src/domain/event.rs src/persistence/projector.rs`; `cargo test project`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` for `CCXProjectPickerTests` compiled touched Swift files/resources and stopped on the existing missing `gui/ghostty` checkout dependency.

...
- 2026-05-25: `gh-review-hook` requested sanitizing `cliProvider()` failures so `.notExecutable(path)` cannot expose filesystem paths. Routed CLI resolution failures through the registration error sanitizer and added test coverage.
- 2026-05-25: Fourth hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the sanitized CLI resolution failure handling.
- 2026-05-25: `gh-review-hook` requested adding `.executableNotFound` to the registration CLI error sanitizer and bounding the duplicate-submit test wait. Added both fixes and explicit coverage for safe CLI-not-found messaging.
- 2026-05-25: Fifth hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the `.executableNotFound` sanitizer and bounded-wait test update.
- 2026-05-25: `gh-review-hook` requested avoiding filesystem validation from render-time `validationError` access and sanitizing `.timedOut`. Converted registration validation to explicit cached state, refreshed it on submit and picker selections, added timeout sanitizer coverage, and kept submit disabled for empty paths without synchronous filesystem probes.
- 2026-05-25: Sixth hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the cached validation and timeout sanitizer updates.
- 2026-05-25: `gh-review-hook` requested completing the registration CLI error sanitizer for `.invalidJSON` and `.cancelled`. Replaced the per-case chain with an exhaustive `CCXControllerCLIError` switch and added coverage for the remaining safe registration failure copy.
- 2026-05-25: Seventh hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the exhaustive CLI sanitizer switch.
- 2026-05-25: `gh-review-hook` requested making the non-CLI error fallback generic, avoiding empty repository paths for the task-source file picker directory, and removing the obsolete add-project placeholder string. Applied all three and added fallback sanitizer coverage.
- 2026-05-25: Eighth hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the generic fallback, picker directory guard, and obsolete string removal.
- 2026-05-25: `gh-review-hook` requested explicit `@MainActor` isolation on the async repository and task-source picker helpers. Annotated both helpers so AppKit panel creation and registration view model mutation remain main-actor isolated across awaits.
- 2026-05-25: Ninth hook-fix validation passed: `git diff --check`; JSON parse for `gui/Resources/Localizable.xcstrings`; `rtk plutil -lint gui/cmux.xcodeproj/project.pbxproj`; `rtk bash gui/scripts/lint-pbxproj-test-wiring.sh --repo-root gui`. Targeted `xcodebuild` compiled touched Swift/test files and again stopped before test execution on the existing missing `gui/ghostty` checkout dependency. Subagent review approved the picker helper actor isolation annotations.

...
- [ ] **1.2 共通エラー型 `CcxError` の定義 (`src/error.rs`)**
  - [ ] `InvalidStateTransition { from, to }`
  - [ ] `WriteLeaseConflict { active_session_id }`
  - [ ] `MergeLockConflict`
  - [ ] `TaskFileConflict` (同時編集競合発生時)
  - [ ] その他 I/O、Git、DB 関連のエラーを網羅
- [ ] **1.3 ULID 生成ユーティリティの実装 (`src/domain/event.rs`)**
  - [ ] Monotonic ULID を生成する `generate_event_id()` を定義
- [ ] **1.4 パス導出ユーティリティとプロジェクト登録コマンド (`src/cli/project.rs`)**
  - [ ] `ccx project register` コマンドの実装
  - [ ] `project.json` への永続化機能の実装

---

### Phase 2: Event 基盤 & JSONL 監査ログの実装

- [ ] **2.1 Event 型および EventType の定義 (`src/domain/event.rs`)**
  - [ ] 3.1節で定義したすべての `EventType` 列挙型の追加
- [ ] **2.2 JSONL append 機能の実装 (`src/persistence/jsonl.rs`)**
  - [ ] `fd-lock` を用いた、プロジェクト固有の `events.lock` に対する排他書き込み処理の実装
  - [ ] シリアライズされた JSON 文字列の追記、`flush()`, `sync_all()` 呼び出しの実装

---

### Phase 3: SQLite Projection (射影器) & Rebuild 機能

- [ ] **3.1 SQLite DDL のマイグレーション実行 (`src/persistence/sqlite.rs`)**
  - [ ] 2.4節で定義した CHECK 制約、UNIQUE 部分インデックス、`artifact_state` カラムを含む DDL の実装
- [ ] **3.2 Projector (射影器) の実装 (`src/persistence/projector.rs`)**
  - [ ] `Event` の受信時に SQLite の状態を変更するトランザクションロジックの実装
  - [ ] `sqlite_dirty` フラグ管理の実装（書き込み失敗時に `state/sqlite.dirty` マーカーファイルを作成）
- [ ] **3.3 Rebuild & Verify の実装 (`src/persistence/rebuild.rs`)**
  - [ ] `ccx db rebuild` コマンドの実装。SQLite ファイルを初期化し、JSONL から状態を完全にロールフォワードする処理（完了後にマーカーファイルを削除）
  - [ ] `ccx db verify` コマンドの実装。SQLite の整合性と JSONL の最新状態をクロスチェック

---

### Phase 4: Domain State (状態遷移 Validator)

- [ ] **4.1 状態遷移 Validator の実装 (`src/domain/transition.rs`)**
  - [ ] 3.2節の `validate_transition` の実装と単体テストの作成
  - [ ] 不正な状態遷移を試みた場合に `CcxError::InvalidStateTransition` を返すか検証

---

### Phase 5: task.md (主成果物) & File Watcher 監視

- [ ] **5.1 task.md Markdown テンプレートおよび YAML front matter 解析**
  - [ ] `serde_yaml` を用いた front matter パース関数の実装
  - [ ] パーサーに不完全なデータや不正な値が渡された場合の防御的バリデーション処理
- [ ] **5.2 task_watcher 実装 (`src/watcher/task_watcher.rs`)**
  - [ ] 各 WorkExecution の `task.md` (`work-executions/<id>/task.md`) の更新（変更検出）を監視する。
  - [ ] コンテンツのコンテンツハッシュ (SHA-256) を計算し、`last_seen_hash` との重複検知 (Deduplication) を行う仕様の実装。同一ハッシュならイベント生成をスキップ。
  - [ ] front matter を best effort で解析する。
  - [ ] `work_execution_task_file_changed` イベントを生成し、JSONL（監査ログ正本）へ append する。
  - [ ] status に変化がない場合は、通知の優先度 (priority) を下げる、または不要な通知をスキップするフィルタリング処理の実装。
  - [ ] Projector がイベントを受けて SQLite に反映（射影）し、最終的に Orchestrator Agent に変更を通知（チャネル経由など）する。
- [ ] **5.3 source_watcher 実装 (`src/watcher/source_watcher.rs`)**
  - [ ] 業務正本である task source file の更新を検知し、`task_source_file_changed` イベントを記録して Orchestrator に伝える仕組みの実装

---

### Phase 6: Git Local 操作モジュール

- [ ] **6.1 canonical repo 制御 (`src/git/repo.rs`)**
  - [ ] canonical repo 用の dirty check (`git status --porcelain` 実行と解析)
- [ ] **6.2 worktree & branch 管理の実装 (`src/git/worktree.rs`)**
  - [ ] リポジトリ外への `git worktree add` およびブランチ生成（作成時に `worktree_created`, `branch_created` イベントを JSONL に append。また、worktree から管理下の `task.md` への symlink 設置処理も含む）
  - [ ] cleanup 時に worktree を削除し、prune を行う処理の実装

---

### Phase 7: CLI Skeleton の構築

- [ ] **7.1 Clap サブコマンド骨格の実装**
  - [ ] `main.rs` および `src/cli/` 以下にすべてのサブコマンドのパースインターフェースを実装
  - [ ] `work create` / `work cleanup` / `lease acquire` / `lease release` / `merge execute` / `recovery digest` を含むすべてのコマンドがダミーデータを `--json` で出力できるスケルトンを作成

---

### Phase 8: Agent Runtime (cmux / tmux アダプター & 協調ライフサイクル)

- [ ] **8.1 Trait `CmuxAdapter` の定義と Unix Socket / CLI 実装**
  - [ ] `src/agent_runtime/cmux_adapter.rs` の作成および 4.4.3 節の Trait 定義
  - [ ] Unix ドメインソケット `/tmp/cmux.sock` への JSON-RPC クライアント実装 (ワークスペース作成、タブ追加、タブ閉鎖、通知)
  - [ ] `cmux` CLI 経由のコマンド発行によるフォールバック動作の実装
  - [ ] macOS 以外の環境や `cmux` アプリ未起動時に、エラーを出さずに headless 実行へ移行するフォールバックロジックの実装
- [ ] **8.2 バックグラウンド tmux アダプターと Harness 実装**
  - [ ] `tmux_adapter.rs` によるバックグラウンド tmux session の起動、アタッチ、停止の実装
  - [ ] `harness.rs` による子プロセスの PID、cwd 監視、および PTY I/O 等に基づく heartbeat 更新
  - [ ] `agent attach` 実行時に、所定の 9 つの環境変数を子プロセス環境に注入する処理の実装
- [ ] **8.3 cmux / tmux の協調ライフサイクル制御**
  - [ ] 新規セッション開始時、まず `tmux` バックグラウンドセッションを作成し、続いて `cmux` タブを開いて `tmux attach` を走らせる二段階起動シーケンスの実装
  - [ ] `ccx agent stop` 時、または cmux タブが閉じられたことを検知した際に、対応する tmux セッションも安全に終了させる同期ロジック
- [ ] **8.4 プロンプト送信・ファイル注入の実装 (`src/agent_runtime/prompt.rs`)**
  - [ ] `--message-file` および `--stdin` を読み込み、対象 tmux 端末へキー入力送信する処理の実装
- [ ] **8.5 Cmux UI 連携とユーザー通知機能 (`ccx agent notify`)**
  - [ ] AI エージェントの作業状況、または `user_intervention` 要求を cmux の UI に通知する `ui.notify` API 連携の実装
  - [ ] CLI サブコマンド `ccx agent notify --session-id <id> --message <msg> [--level <info|warning>]` の追加
- [ ] **8.6 Agent lifecycle hook / exit hook の抽象化と実装**
  - [ ] Agent 実装固有の stop / end event を `AgentLifecycleStop` イベントに写像する。
  - [ ] lifecycle hook 発火時に `task.md` の存在および非空（0バイトでないこと）を確認・検証する。
  - [ ] 検証成功時に `AgentLifecycleStop` イベントを JSONL 監査ログに記録し、かつ SQLite の `artifact_state` を `ready` に（失敗時は `invalid` に）投影更新する。
  - [ ] イベント発生を Orchestrator Agent に通知する。

---

### Phase 9: PR / Hook / マージ自動化フロー

- [ ] **9.1 gh-review-hook 実行の実装 (`src/git/review_hook.rs`)**
  - [ ] worktree を cwd としたフック起動と exit code (`0`, `2`, その他) の分類評価
- [ ] **9.2 マージ実行アルゴリズムの実装 (`src/git/github.rs`)**
  - [ ] 14.4節の「マージ実行フロー」に基づくマージトランザクションの実装
  - [ ] 4.3節の「安全な Canonical Repo 同期処理」の実装。sync_warning の制御ロジック

---

### Phase 10: 自律復旧 & Circuit Breaker

- [ ] **10.1 `recovery digest` 機能の実装**
  - [ ] 整合性チェック（SQLite情報と tmux 端末/PID/Git状態のクロスチェック）
  - [ ] 孤立セッション自動検出、stale な write lease や merge lock の検出処理
- [ ] **10.2 Circuit Breaker 機能の実装**
  - [ ] リトライ回数閾値・実行時間の自動評価、閾値到達時の `hold` 自動遷移

---

### Phase 11: クリーンアップポリシー制御

- [ ] **11.1 クリーンアップポリシーの実装 (`src/cli/work.rs`)**
  - [ ] `cleanup_policy` (immediate | keep_last_n | keep_for_duration) の設定に応じた、アタッチセッション (tmux/cmux) および worktree の破棄ロジックの実装

---

### Phase 12: 3段階 E2E テストと検証

- [ ] **12.1 Level 1: Pure / Local Tests**
  - [ ] `validate_transition` 状態遷移の単体テスト
  - [ ] JSONL ファイル書き込みロック、SQLite マイグレーション、front matter パーサーの単体テスト
- [ ] **12.2 Level 2: Local Git Tests**
  - [ ] ローカルの git ベアリポジトリに対する worktree の作成・削除、ブランチ作成、dirty check、reset のテスト
- [ ] **12.3 Level 3: Fake External Command Tests**
  - [ ] テスト用フィクスチャとして `tests/fixtures/bin/gh` および `tests/fixtures/bin/gh-review-hook` のフェイクスクリプトを作成
  - [ ] `gh pr view` や `gh pr merge` のダミー応答を介して、PR状態・SHAの不一致・フック exit code に応じるコントローラーの一連の挙動（E2E）を全自動でテストするスクリプトを構築
- [ ] **12.4 本番 GitHub 結合テストの実行 (マニュアル実行 / Nightly 用)**
  - [ ] 実際の GitHub に対して PR の作成・フック検証・自動マージができるかを手動検証

---

### Phase 13: cmux フォークによる CCX 専用 GUI の実装

- [ ] **13.1 cmux リポジトリのフォークおよびビルド環境の構築**
  - [ ] `cmux` (Swift/AppKit/libghostty ベース of macOS アプリ) のソースコードをフォークし、Xcode を用いたローカル開発・ビルド環境をセットアップする
- [ ] **13.2 コントローラーおよび永続化データとのデータ連携**
  - [ ] フォークした Swift アプリケーション内に、CCX Controller が生成する SQLite (`state.sqlite`) および JSONL ログファイルを読み込み・監視する機能を実装する
- [ ] **13.3 CCX 専用 ダッシュボード / サイドバー GUI の実装 (SwiftUI/AppKit)**
  - [ ] `ccx_requirements_parallelized_v3.md` の「7.2 表示領域」に準拠した統合管理画面を実装する
    - [ ] **Overview**: プロジェクトの状態、アクティブエージェント数、Hold/Merge_ready セッション数などのメタ情報をリアルタイム表示するダッシュボード
    - [ ] **WorkExecutions / AgentSessions**: 現在稼働中の Worktree、ブランチ、各セッションの役割、アクティブな write lease 状態の可視化
    - [ ] **Reviews / Merges**: 自動レビュー結果 (gh-review-hook のステータス・エラーログ) およびマージキューの表示
    - [ ] **Artifacts / Events**: task.md、差分要約、および JSONL から射影されたタイムライン表示
- [ ] **13.4 カスタムタブ / マルチ Surface の統合レイアウト**
  - [ ] 各 AgentSession に対応する Ghostty ターミナルサーフェスと、作業タスク進捗状況などを一覧できるサイドパネルを、美しくレスポンシブなスプリット表示として統合
- [ ] **13.5 コントローラー CLI からのシームレスな起動連携**
  - [ ] `ccx project open` コマンドが実行された際に、フォークした CCX-cmux アプリを `open -a ccx-cmux --args --project-id <id>` のように引数付きで呼び出し、自動で当該プロジェクトのワークスペースを開く起動連携を実装する

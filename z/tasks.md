...
- [x] **14.2 全プロジェクト一覧モデル (`CCXProjectsStore`)**
  - [x] `~/.ccx/projects.json` を `FSEventStream` で監視し、プロジェクト追加・削除を即時反映する `ObservableObject` を新設
  - [x] 既存 `CCXProjectStore` (単一プロジェクト) と同様、I/O はバックグラウンドキュー、`@Published` 更新のみ MainActor

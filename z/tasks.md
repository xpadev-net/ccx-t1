...
- [ ] **13.6 (Follow-up) Xcode / cmux 本体への組み込み**
  - [ ] `gui/Sources/CCX/*.swift` を `gui/cmux.xcodeproj/project.pbxproj` の main app target に組み込み (PBXFileReference / PBXBuildFile / PBXSourcesBuildPhase) して実ビルドを通す
  - [ ] cmux 既存の `PanelType` enum に CCX サイドバー panel を追加し、 `AppDelegate` / `ContentView` から `CCXLaunchArguments.parse()` 経由でダッシュボードを開けるよう配線する
  - [ ] `PRODUCT_NAME` / `PRODUCT_BUNDLE_IDENTIFIER` を `ccx-cmux` 系へリネームし、合わせて `gui/scripts/reload*.sh` および関連ドキュメントを更新する

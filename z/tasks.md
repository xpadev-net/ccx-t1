...
- [ ] **15.2 Task Source 読み書き CLI**
  - [ ] `ccx task-source read --project-id <id> --json` を追加し、path、content、hash、mtime を返す
  - [ ] `ccx task-source write --project-id <id> --expected-hash <hash> --stdin --json` を追加し、競合時は non-zero と明示エラーを返す
  - [ ] `ccx task-source append --project-id <id> --expected-hash <hash> --stdin --json` を追加し、追記位置と更新後 hash を返す
  - [ ] task source file が canonical repo 内にある場合、dirty state が発生することを JSON warning として返す
- [ ] **15.3 GUI 手動編集ビュー**
  - [ ] `CCXControllerCLI` に task source read/write/append の型安全 wrapper を追加する
  - [ ] `CCXTaskSourceStore` を追加し、読み込み、保存、競合検知、再読込を MainActor 安全に扱う
  - [ ] Tasks タブに Markdown テキスト編集領域、保存、再読込、変更破棄の導線を実装する
  - [ ] 保存時に `expected-hash` 不一致なら上書きせず、差分確認または再読込を促す
- [ ] **15.4 自然言語 Task 追加 Composer**
  - [ ] Tasks タブに自然言語入力欄を追加し、「LLM で詳細化して追加」操作を提供する
  - [ ] 入力には task source file、canonical repo、現在の WorkExecution 状態、希望する追加形式を含めた Orchestrator 向け prompt template を適用する
  - [ ] Orchestrator Agent session が存在しない場合は開始し、存在する場合は `ccx agent prompt` で依頼を投入する
  - [ ] prompt には「コードを確認し、必要なら task を分割・詳細化し、task source file に反映する。GUI からの原文も残す」ことを明記する
- [ ] **15.5 Orchestrator 反映フロー**
  - [ ] Orchestrator Agent が task source file 更新後、Controller の source watcher が `task_source_file_changed` を記録し、GUI が再読込できることを確認する
  - [ ] LLM 詳細化の結果として追記された task の見出し / チェックボックス / anchor を GUI で確認できるようにする
  - [ ] 失敗時は Orchestrator Agent からのエラー、未反映理由、追加で必要な情報を GUI に表示する
- [ ] **15.6 WorkExecution 作成連携**
  - [ ] GUI 上で task source file の見出し / チェックボックス / 選択範囲を選び、`ccx work create` に渡せるようにする
  - [ ] `work create` の skeleton を実装し、WorkExecution、task.md、branch、worktree、イベント、SQLite projection が実際に作成されるようにする
  - [ ] 作成した WorkExecution に Orchestrator / Worker Agent を attach / prompt できる導線を追加する
- [ ] **15.7 検証**
  - [ ] CLI 単体テスト: read/write/append、hash 競合、missing file、dirty warning
  - [ ] GUI unit test: Tasks タブ状態、保存成功、競合表示、Composer prompt 生成
  - [ ] fake `ccx` / fake Orchestrator で自然言語 task 追加の E2E を通す

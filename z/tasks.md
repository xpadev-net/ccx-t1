...
- [ ] **13.7 (Follow-up) ccx-cmux ブランド化**
  - [ ] `PRODUCT_NAME` / `PRODUCT_BUNDLE_IDENTIFIER` を `ccx-cmux` 系へリネームし、合わせて `gui/scripts/reload*.sh`、`gui/scripts/cmux-debug-cli.sh`、`gui/CLAUDE.md` および関連ドキュメントを更新する (Xcode + ghostty submodule を要するため後続セッションで実施)
  - [ ] `AppDelegate.applicationDidFinishLaunching` から `CCXLaunchArguments.parse()` 経由で CCX ダッシュボードを開く起動フックを差し込む

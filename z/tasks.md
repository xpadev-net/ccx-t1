...
- [ ] **14.1 CLI ランチャー抽象 (`gui/Sources/CCX/CCXControllerCLI.swift`)**
  - [ ] `ccx` バイナリのパスを解決する (起動時の `CCX_CLI` 環境変数 → `$CCX_HOME/bin/ccx` → `PATH` 探索の優先順)。バイナリが見つからない場合はエラー表示用の Result を返す
  - [ ] `register(canonicalRepo:taskSourceFile:) async throws -> CCXProjectSummary` のように `--json` 出力をパースする型安全ラッパーを実装
  - [ ] サブプロセス stderr を保持し、CLI 失敗時に GUI 側のエラートーストで原因を提示する
- [ ] **14.2 全プロジェクト一覧モデル (`CCXProjectsStore`)**
  - [ ] `~/.ccx/projects.json` を `FSEventStream` で監視し、プロジェクト追加・削除を即時反映する `ObservableObject` を新設
  - [ ] 既存 `CCXProjectStore` (単一プロジェクト) と同様、I/O はバックグラウンドキュー、`@Published` 更新のみ MainActor
  - [ ] 各 entry を `CCXProjectSummary` として公開し、起動済みの単一プロジェクトダッシュボードと相互運用
- [ ] **14.3 Project Picker サイドバー / Welcome 画面**
  - [ ] `--project-id` 無しで起動した場合に表示する Project Picker を実装 (プロジェクト一覧、選択で新規 `CCXDashboardPanel` を adopt、"+ Add Project" 行で 14.4 のシートを表示)
  - [ ] サイドバー上部に現在のプロジェクト切替メニュー (NSMenu または SwiftUI Menu) を追加し、選択時は新しいタブで該当プロジェクトのダッシュボードを開く
- [ ] **14.4 新規プロジェクト登録 Sheet**
  - [ ] `NSOpenPanel` でリポジトリパスと task source file (`*.md`) を選択させるフォーム UI を実装
  - [ ] バリデーション (`.git` ディレクトリ存在 / task source file 存在) を GUI 側で先行チェックし、`ccx project register --json` をサブプロセス実行して結果を取り込む
  - [ ] 登録成功時、即座に新規プロジェクトのダッシュボードに切り替える
- [ ] **14.5 プロジェクト切替 (`Workspace.switchToCCXDashboard(projectId:)`)**
  - [ ] 既存ワークスペース内の `CCXDashboardPanel` を破棄して別 projectId のパネルに置換するヘルパーを `Workspace` に追加
  - [ ] サイドバー切替時にこのヘルパーを呼び、`publishCmuxSurfaceClosed` → `newCCXDashboardSurface` を順序保証して発火
- [ ] **14.6 プロジェクト削除 / Unregister**
  - [ ] サイドバー行の右クリックメニューに "Unregister project" を追加し、確認ダイアログ後に `ccx project unregister <id>` をサブプロセス起動 (まだ未実装なので CLI 側にも追加が必要)
  - [ ] Controller CLI 側に `ccx project unregister --project-id <id> [--purge]` を実装し、`projects.json` から該当行削除、`~/.ccx/projects/<id>/` を `--purge` 指定時のみ削除、events.jsonl に `project_unregistered` を append
- [ ] **14.7 起動引数ポリシーの整理 (`CCXLaunchArguments`)**
  - [ ] `--project-id` 省略時は Project Picker を表示、`--project-id` 指定時は従来通り直接ダッシュボードを開くよう既存の `CCXAppDelegateBridge.presentDashboardIfRequested(on:)` を更新
  - [ ] 環境変数 `CCX_DEFAULT_PROJECT_ID` 経由でデフォルトプロジェクトを設定できるオプションも追加
- [ ] **14.8 実機検証**
  - [ ] 新規ビルドの `ccx-cmux DEV.app` から:
  ...
  - [ ] CLI と GUI を交互に操作しても `projects.json` / `state.sqlite` が破綻しないことを `ccx db verify` で確認
